...
import { Z3WalletConnectedWallet } from "./Z3WalletConnectedWallet";

export const parseBalanceResponse = (
  balanceRPCResponse: RPCResponse<any, string>
) => {
  ...
  try {
    ...
    if (error.message !== "Account is not created") throw error;
    balanceResult = {
      balance: 0,
    };
  }
...
export const connectWalletPrivateKey = async (
...
export const connectWalletZilPay = async (
  zilPay: any
): Promise<ConnectWalletResult> => {
  ...
  if (!network) throw new Error(`Unsupported network for ZilPay: ${net}`);
...
export const connectWalletZ3Wallet = async (
  z3Wallet: any
): Promise<ConnectWalletResult> => {
  if (!z3Wallet.wallet.isConnect)
    throw new Error("Z3Wallet connection failed.");

  const account: any = z3Wallet.wallet.defaultAccount;
  if (!account)
    throw new Error(
      "Please sign in to your Z3Wallet account before connecting."
    );
  const timestamp = dayjs();

  const net = z3Wallet.wallet.net;
  const network = Z3WalletNetworkMap[net];
  if (!network) throw new Error(`Unsupported network for Z3Wallet: ${net}`);

  const wallet = new Z3WalletConnectedWallet({
    network,
    timestamp,
    z3wallet: z3Wallet as WalletProvider,
    bech32: account!.bech32,
    base16: account!.base16,
  });
...
export const connectWalletBoltX = async (
  boltX: any
): Promise<ConnectWalletResult> => {
  ...
  if (!network) throw new Error(`Unsupported network for BoltX: ${net}`);
...
export const connectWalletZeeves = async (
  zeeves: any
): Promise<ConnectWalletResult> => {
  try {
    ...
    if (!network) throw new Error(`Unsupported Zeeves network: ${net}`);
