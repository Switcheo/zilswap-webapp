...
const ConnectedWalletBox = (props: any) => {
  ...
  useEffect(() => {
    if (wallet && wallet.wallet) wallet.wallet.reload();
  }, [])
  ...
  return (
    <Box display="flex" flexDirection="column">
      <ContrastBox className={cls(classes.root, className)}>
        ...
        <Box className={classes.label}>
          ...
          <Box mt={"8px"} display="flex" flexDirection="row" alignItems="center">
            <Typography color="textSecondary" variant="body1">{wallet.wallet.account.address}</Typography>
            <IconButton target="_blank" href={`https://viewblock.io/zilliqa/address/${wallet.wallet.account.address}`} className={classes.newLink} size="small"><NewLinkIcon /></IconButton>
            <IconButton onClick={() => navigator.clipboard.writeText(wallet.wallet.account.address)} className={classes.copy} size="small"><CopyIcon /></IconButton>
          </Box>
      ...
      <Box mt={"36px"}>
        ...
        {wallet && wallet.wallet && wallet.wallet.transactions && wallet.wallet.transactions.map((transaction: any) => (
          <Box key={transaction.hash}>
            <Box display="flex" flexDirection="row" justifyContent="space-between">
              <Box display="flex" flexDirection="row" alignItems="center">
                <Typography variant="body2">{truncate(transaction.hash, 10, 10)}</Typography>
                <IconButton target="_blank" href={`https://viewblock.io/zilliqa/tx/${transaction.hash}`} className={classes.newLinkTransaction} size="small"><NewLinkIcon /></IconButton>
              </Box>
