...
const RewardsInfoButton: React.FC<Props> = (props: Props) => {
  ...
  const [selectedRewardsValue, setSelectedRewardsValue] = useState<BigNumber>(
    new BigNumber(0)
  );
  ...
  useEffect(() => {
    ...
    runLoadBears(async () => {
      ...
      const query: ArkClient.ListTokenParams = {
        owner: walletState.wallet?.addressInfo.byte20.toLowerCase(),
        limit: 500,
      };
  ...
  const claimableRewards: ReadonlyArray<ClaimableRewards> = distributions
    ...
    .flatMap((d: DistributionWithStatus) => {
      const rewardDistributor = distributors.find(
        distributor => distributor.distributor_address_hex === d.info.distributor_address
      );
      ...
      const rewardTokenAddress =
        'reward_token_address' in d.info
          ? d.info.reward_token_address
  ...
  const onClaimRewards = () => {
    runClaimRewards(async () => {
      ...
      if (selectedDistributions.length === 0) {
        if (showDetails) {
          setSelectedDistributions(
            claimableRewards.filter(
              r => r.funded && !claimedDistributions.includes(r.info.id)
            )
          );
        } else {
          setShowDetails(true);
      ...
      const filteredRewards = claimableRewards.filter(
        r => !!selectedDistributions.find(d => d.info.id === r.info.id)
      );
      ...
      const distributions = selectedDistributions.map(distribution => {
        ...
        const proof = distribution.isTbmFee
          ? distribution.info.proof
          : distribution.info.proof.slice(1, distribution.info.proof.length - 1);
  ...
  const isAllSelected = useMemo(() => {
    return (
      claimableRewards.length === selectedDistributions.length &&
          ...
          <ClickAwayListener onClickAway={() => setActive(false)}>
            <Card className={classes.card}>
              <Box
                ...
                gridRowGap="18px"
              >
                <Text variant="h6" color="textPrimary" className={classes.header}>
                  Your Balance
                </Text>
                {claimSuccess && (
                  <>
                    ...
                    <Button
                      ...
                      onClick={() => resetClaimedRewardStates()}
                    >
                ...
                {!claimSuccess && (
                  ...
                  >
                    <Box
                      ...
                      flexDirection="column"
                      alignItems="center"
                      className={classes.balanceFrame}
                    >
                      <CurrencyLogo
                        currency="ZWAP"
                        address={zwapAddress}
                  ...
                  <Box display="flex" flexDirection="column" alignItems="center" mt={2}>
                    <Text className={classes.body}>
                      ...
                      <HelpInfo
                        ...
                        title={
                          <span>
                            The estimated amount of rewards you have collected and are
                            eligible to claim (includes <b>ARKY</b> and{' '}
                            <b>liquidity mining</b> rewards).
                          </span>
                        }
                        className={classes.tooltip}
                        additionalStyling={classes.tooltipMaxWidth}
                      />
                    </Text>
                    {claimableRewards.length !== 0 && (
                      <Box
                      ...
                      >
                        {Object.keys(claimableAmountsByToken).map(tokenAddress => {
                          ...
                          return (
                            ...
                            >
                              {displayRewardAmount(
                                claimableAmountsByToken[tokenAddress].shiftedBy(
                                  -token!.decimals
                                )
                              )}
                              &nbsp;
                              ...
                              />
                              &nbsp;
                              <span className={classes.currency}>{token.symbol}</span>
                            </Text>
                          );
                        })}
                        <Text
                                  ...
                                  return (
                                    <Box mt={1} key={date}>
                                      ...
                                      {rewardsByDate[date].map(reward => {
                                        ...
                                        const isDisabled =
                                          !reward.funded ||
                                          claimedDistributions.includes(reward.info.id);
                                                ...
                                                <Text className={classes.epochReward}>
                                                  {reward.isTbmFee && (
                                                    <img
                                                      src={ArkyLogo}
                                                      alt="arkyLogo"
                                                      className={classes.arkyLogo}
                                                    />
                                                  )}
                                                  {displayRewardAmount(
                                                    reward.info.amount.shiftedBy(
                                                      -token.decimals
                                                    )
                                                  )}
                                                  &nbsp;
                    ...
                    <Tooltip title={claimTooltip}>
                      <span>
                        <Button
                          ...
                          variant="contained"
                          color="primary"
                          disabled={claimableRewards.length === 0 || loading || txLoading}
