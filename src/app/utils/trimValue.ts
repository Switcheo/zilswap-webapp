...
const trimInputValue = (inputValue: BigNumber | string, toDecimalPlace = 5, ellipsis = false) => {
  ...
  if (value.isGreaterThan(1)) {
    if (decLen > toDecimalPlace) return value.toFixed(toDecimalPlace) + ellipsisStr;
    ...
    return value.toFixed(decPlace + toDecimalPlace) + ellipsisStr;
  }
}
