...
import { getConnectedZ3Wallet } from "core/utilities/z3wallet";
import {
  PoolTransaction,
...
const z3WalletObserver = (z3Wallet: any) => {
  return eventChannel<ConnectedWallet>((emitter) => {
    const accountObserver = z3Wallet.wallet.observableAccount();
    const networkObserver = z3Wallet.wallet.observableNetwork();

    accountObserver.subscribe(async (account: any) => {
      logger(`Z3Wallet account changed to: ${account.bech32}`);
    ...
    networkObserver.subscribe(async (net: string) => {
      logger(`Z3Wallet network changed to: ${net}`);
      const walletResult = await connectWalletZ3Wallet(z3Wallet);
      if (walletResult?.wallet) {
        emitter(walletResult.wallet);
      }
    ...
    logger("registered z3wallet observer");

    return () => {
      logger("deregistered z3wallet observer");
...
function* txObserved(payload: TxObservedPayload) {
  ...
  detachedToast(`Transaction ${status ? status : "confirmed"}`, {
    hash: tx.hash,
  });
  ...
  if (
    transactions.find(
      (transaction: Transaction) => transaction.hash === tx.hash
    )
  ) {
  ...
  try {
    ...
    try {
      ...
      Object.entries(mappings).forEach(([wrappedDenom, sourceDenom]) => {
        ...
        const wrappedToken = carbonTokens.find(
          (d) => d.denom === wrappedDenom
        )!;
        const sourceToken = carbonTokens.find((d) => d.denom === sourceDenom)!;

        const wrappedChain = blockchainForChainId(
          wrappedToken.chainId.toNumber()
        );
        const sourceChain = blockchainForChainId(
          sourceToken.chainId.toNumber()
        );

        if (
          (wrappedChain !== Blockchain.Zilliqa &&
            wrappedChain !== Blockchain.Ethereum) ||
          (sourceChain !== Blockchain.Zilliqa &&
...
function* watchReloadPoolTx() {
  while (true) {
    try {
      ...
      if (wallet) {
        const result: PoolTransactionResult = yield call(
          ZAPStats.getPoolTransactions,
          {
            network: network,
    ...
    try {
      ...
      if (action.payload.wallet?.type === WalletConnectType.ZilPay) {
        ...
        chan = (yield call(
          zilPayObserver,
          zilPay
        )) as EventChannel<ConnectedWallet>;
        break;
      }
    } catch (e) {
      console.warn(
        "Watch Zilpay failed, will automatically retry on reconnect. Error:"
      );
      console.warn(e);
    }
  }
...
function* watchZ3Wallet() {
  let chan;
  while (true) {
    try {
      const action: WalletAction = yield take(WalletActionTypes.WALLET_UPDATE);
      if (action.payload.wallet?.type === WalletConnectType.Z3Wallet) {
        logger("starting to watch z3wallet");
        const z3Wallet = (yield call(getConnectedZ3Wallet)) as unknown as any;
        chan = (yield call(
          z3WalletObserver,
          z3Wallet
        )) as EventChannel<ConnectedWallet>;
      ...
      console.warn(
        "Watch Z3Wallet failed, will automatically retry on reconnect. Error:"
      );
      console.warn(e);
    }
  }
  try {
    while (true) {
      const newWallet = (yield take(chan)) as ConnectedWallet;
      const { wallet: oldWallet } = getWallet(yield select());
      if (oldWallet?.type !== WalletConnectType.Z3Wallet) continue;
      if (
        newWallet.addressInfo.bech32 === oldWallet?.addressInfo.bech32 &&
        newWallet.network === oldWallet.network
      )
...
function* watchBoltX() {
  ...
  while (true) {
    try {
      ...
      console.warn(
        "Watch BoltX failed, will automatically retry on reconnect. Error:"
      );
      console.warn(e);
    }
  }
...
function* watchWeb3() {
  ...
  while (true) {
    try {
      const action: BridgeWalletAction = yield take(
        WalletActionTypes.SET_BRIDGE_WALLET
      );
      ...
      console.warn(
        "Watch web3 failed, will automatically retry to reconnect. Error:"
      );
      console.warn(e);
    }
  }
