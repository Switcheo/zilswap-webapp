...
const SwapTxRow: React.FC<Props> = (props: Props) => {
  ...
  const { isError, inToken, outToken, swapRoute, totalValue, inAmount, outAmount } =
    React.useMemo(() => {
  ...
  return (
    <TableRow {...rest} className={cls(classes.root, className)}>
        ...
        <Box display="flex">
          ...
          <Box className="external-link" marginLeft={1}>
            <a
              ...
              href={`https://viewblock.io/zilliqa/tx/${
      ...
      <TableCell>
        <Box display="flex" alignItems="center">
          ...
          <Text className={classes.text}>
            {swapRoute.map(token => token.symbol).join(' - ')}
          </Text>
        </Box>
      </TableCell>
      <TableCell align="right">${totalValue.toFormat(2)}</TableCell>
      ...
      <TableCell align="right">{transaction.block_timestamp.fromNow()}</TableCell>
      <TableCell className={classes.placeholderCell} />
    </TableRow>
  );
};
